...
- **不恢复动态工具**:官方在本次更新中明确「移除原 Cordis 动态定义及运行工具,调整为通过 Plugin Manager 安装持久化插件」,本预设对齐该方向,只保留官方 `tool-cordis` 的只读检查工具。动态模型原先唯一不可替代的「免重启迭代」,现在持久化路径也已具备:新 bundle 经 HMR 激活,改完代码后关闭再开启该 bundle 即生效,`remove_bundle` 支持运行时卸载。历史会话中调用过这些工具的记录不会损坏——工具只在调用瞬间解析,回放时缺失的工具走通用卡片兜底。
...
- 共存 shim 依赖宿主平面的 `dynamicCordisRunner` / `cordisInspect` 服务(随 DSH web 部署自带);在不提供 host runner 的部署里,`cordis-tools` 组内的行会处于等待状态。
